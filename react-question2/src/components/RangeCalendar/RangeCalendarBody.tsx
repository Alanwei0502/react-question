...
const RangeCalendarBody: React.FC<RrangeCalenderBodyProps> = (props) => {
  ...
  const lastMonthDate = Array.from({ length: firstDayInThisMonth }, (_, i) =>
    selectedMonth
      .subtract(1, 'M')
      .endOf('M')
      .add(-firstDayInThisMonth + i + 1, 'd'),
  );

  const thisMonthDate = Array.from({ length: daysInThisMonth }, (_, i) =>
    selectedMonth.startOf('M').add(i, 'd'),
  );

  const nextMonthDate = Array.from(
    { length: 42 - lastMonthDate.length - thisMonthDate.length },
    (_, i) => selectedMonth.add(1, 'M').startOf('M').add(i, 'd'),
  );
