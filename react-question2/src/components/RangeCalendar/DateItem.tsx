...
const DateItem: React.FC<DateItemProps> = (props) => {
  ...
  const isActive = () => {
    ...
    const isSame = value[0].isSame(item, 'date') || value[1].isSame(item, 'date');
  ...
  return (
    <div className={`${classes['calendar-body_date_item']} ${isNotCurrentMonth ? classes['non-current-month'] : ''} ${isToday ? classes['today'] : ''} ${isActive() ? classes['active'] : ''}`} style={sx} onClick={handleSelectDate}>{item.date()}</div>
  )
}
